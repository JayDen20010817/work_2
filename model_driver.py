...
TEXT_INPUT_PATH = 'main/Data/Text/'
ACOUSTIC_INPUT_PATH = 'main/Data/Audio/'
VISUAL_INPUT_PATH = 'main/Data/Video/'
...
# ---------------------------------------------- Dynamic Context Guiding-Broadcasting (DCGB) ----------------------------------------------
class Context_Self_embedding_Attention(nn.Module):

    def __init__(self,
                 dim_model: int,
                 dim_context: int,
                 dropout_rate: Optional[float]=0.0):
        super(Context_Self_embedding_Attention, self).__init__()        
...
class MIF(nn.Module):
    ...
    def __init__(self,
        ...
        self.acoustic_context_attention = Context_Self_embedding_Attention(dim_model=dim_model,
                                                                dim_context=ACOUSTIC_DIM,
                                                                dropout_rate=dropout_rate)
        self.visual_context_attention = Context_Self_embedding_Attention(dim_model=dim_model,
    ...
    def forward(self,
        ...
        subspace = SubSpace(nin=154)   

        # Video as Context for Attention
        visual_context = visual_context.permute(1, 0)
        visual_context = self.visual_context_transform(visual_context)
        visual_context = visual_context.permute(1, 0)
        #torch.Size([480, 2048])

        #-------------------------------Context Boardcasting   visual_context-------------
        visual_context=0.5*visual_context+0.5*visual_context.mean(dim=1,keepdim=True)

        #audio_out  torch.Size([16, 480, 768])



        #Text-Video Context Self-embedding Attention
        video_out = self.visual_context_attention(q=text_input,
                                                  k=text_input,
                                                  v=text_input,
                                                  context=visual_context)         
        # Audio as Context for Attention
        acoustic_context = acoustic_context.permute(1, 0)
        acoustic_context = self.acoustic_context_transform(acoustic_context)
        #acoustic_context torch.Size([480, 154])
        device = torch.device('cuda:2' if torch.cuda.is_available() else 'cpu')
        acoustic_context = acoustic_context.unsqueeze(0).expand(BATCH_SIZE,-1,-1).unsqueeze(1).expand(-1,154,-1,-1)
        acoustic_context=subspace(acoustic_context).to(device)
        acoustic_context=acoustic_context[:1,:1,:,:].squeeze(1).squeeze(0)


        #Text-Aideo Context Self-embedding Attention
        audio_out = self.acoustic_context_attention(q=text_input,
                                                    k=text_input,
                                                    v=text_input,
                                                    context=acoustic_context)
        
        #Cross-modal Guiding Attention
        video_out_2 = self.visual_context_attention(q=audio_out,
                                            k=text_input,
                                            v=text_input,
                                            context=visual_context)  
        #Cross-modal Guiding Attention
        audio_out_2 = self.acoustic_context_attention(q=video_out,
                                            k=text_input,
                                            v=text_input,
                                            context=acoustic_context)
        ...
        weight_a = F.sigmoid(self.acoustic_gate(torch.cat((audio_out_2, text_input), dim=-1)))
        weight_v = F.sigmoid(self.visual_gate(torch.cat((video_out_2, text_input), dim=-1)))


        output_a = self.final_layer_norm(weight_a * text_input +
                                       weight_a * audio_out_2 + 
                                       video_out_2)
        output_v = self.final_layer_norm(weight_v * text_input +
                            weight_v * video_out_2 + 
                            audio_out_2)


        output = self.final_layer_norm(weight_a * output_a + 
                                       weight_v * output_v)
...
class MultimodalBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # ================================ Modifications ================================ #
        self.fusion_at_layer = [5]#setting layers
        self.visual_transformer = nn.TransformerEncoder(nn.TransformerEncoderLayer(VISUAL_DIM, 8, VISUAL_DIM), 4
                                                     )
        self.acoustic_transformer = nn.TransformerEncoder(nn.TransformerEncoderLayer(ACOUSTIC_DIM, 2, ACOUSTIC_DIM), 4
                                                     )

        self.MIF_layer = MIF(dim_model=embed_dim,
                             dropout_rate=0.2)
        # =============================================================================== #
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusion_at_layer:
                ...
                hidden_states = self.MIF_layer(text_input=hidden_states,
...
def get_val_scores(model,
    ...
    if "Validation" in desc:
        ...
        file_name = RESULT_OUTPUT_DIR + "val/MIF_TAV_BART_epoch_" + str(epoch+1) + "_val_results.csv"
        ...
        file_name = RESULT_OUTPUT_DIR + "test/MIF_TAV_BART_epoch_" + str(epoch+1) + "_test_results.csv"
...
def prepare_for_training(model, 
    ...
    for name, param in model.named_parameters():
        if "acoustic_transformer" or "visual_transformer" or "MIF_layer" in name:
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        path = MODEL_OUTPUT_DIR + "MIF_TAV_BART_epoch_"  + TARGET_COLUMN + "_epoch_" + str(epoch+1) + "_" + datetime.now().strftime('%d-%m-%Y-%H:%M')
        save_model(model,
                   path,
                   tokenizer)
...
if __name__ == "__main__":
    ...
    MODEL = MultimodalBartForConditionalGeneration.from_pretrained('main/bart-base')
    MODEL.to(DEVICE)

    TOKENIZER = BartTokenizerFast.from_pretrained('main/bart-base')
    SOURCE_PREFIX = ''
    TARGET_PREFIX = ''
    ...
    train_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'train_data_final_plain.json',
                                       acosutic_path = ACOUSTIC_INPUT_PATH + 'train_acoustic_features.pkl',
                                       visual_path = VISUAL_INPUT_PATH + 'train_visual_features.pkl',
                                       lowercase_utterances = LOWERCASE_UTTERANCES,
                                       unfolded_dialogue = UNFOLDED_DIALOGUE)
    
    val_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'val_data_final_plain.json',
                                     acosutic_path = ACOUSTIC_INPUT_PATH + 'val_acoustic_features.pkl',
                                     visual_path = VISUAL_INPUT_PATH  + 'val_visual_features.pkl',
                                     lowercase_utterances = LOWERCASE_UTTERANCES,
                                     unfolded_dialogue = UNFOLDED_DIALOGUE)
    
    test_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'test_data_final_plain.json',
                                      acosutic_path = ACOUSTIC_INPUT_PATH + 'test_acoustic_features.pkl',
                                      visual_path = VISUAL_INPUT_PATH  + 'test_visual_features.pkl',
                                      lowercase_utterances = LOWERCASE_UTTERANCES,
                                      unfolded_dialogue = UNFOLDED_DIALOGUE)
    gc.collect()  
    # ------------------------------ TRAINING SETUP ------------------------------ #
